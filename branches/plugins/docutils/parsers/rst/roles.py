...
names are defined in the ``language`` subpackage.  This mapping is deprecated;
see <http://docutils.sf.net/docs/howto/rst-roles.html>."""
...
def _deprecated(fn):
    print >>sys.stderr, \
          'Warning: docutils.parsers.rst.roles.%s is deprecated; ' \
          'see <http://docutils.sf.net/docs/howto/rst-roles.html>.' % fn
...
def role(role_name, language_module, lineno, reporter):
    ...
    # Get the role entry point, and return it.
    try:
        role = utils.get_entry_point('docutils.parsers.rst.roles',
                                     canonicalname)
        register_local_role(normname, role)
        return role, messages
    except utils.EntryPointNotFoundError:
        if _role_registry.has_key(canonicalname):
            _deprecated('_role_registry')
            return _role_registry[canonicalname], messages
...
def register_canonical_role(name, role_fn):
    ...
    """
    _deprecated('register_canonical_role()')
...
def register_generic_role(canonical_name, node_class):
    ...
    _deprecated('register_generic_role()')
...
abbreviation = GenericRole('abbreviation', nodes.abbreviation)
acronym = GenericRole('acronym', nodes.acronym)
emphasis = GenericRole('emphasis', nodes.emphasis)
literal = GenericRole('literal', nodes.literal)
strong = GenericRole('strong', nodes.strong)
subscript = GenericRole('subscript', nodes.subscript)
superscript = GenericRole('superscript', nodes.superscript)
title_reference = GenericRole('title-reference', nodes.title_reference)
